...
export class ThingService {
	...
	getAll(): Observable<Thing[]> {
		return this.http.get<Thing[]>(this.actionUrl, {
			headers: this.headers
		});
	}

	getSingle(id: number): Observable<Thing> {
		return this.http.get<Thing>(this.actionUrl + id, {
			headers: this.headers
		});
	}
	...
	add(thingToAdd: Thing): Observable<Thing> {
		...
		return this.http.post<Thing>(this.actionUrl, toAdd, {
			headers: this.headers
		});
	}

	update(id: number, itemToUpdate: any): Observable<Thing> {
		return this.http.put<Thing>(
			this.actionUrl + id,
