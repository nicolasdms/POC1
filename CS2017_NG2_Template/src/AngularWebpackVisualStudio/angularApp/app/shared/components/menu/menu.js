...
function dw_MenuToMenu(o) {
    return new Menu(o.DWMenuId, o.Text, o.url, '', o.icon, '', '', false, o.DWParentMenuId, false, []);
}
function addChild(parent, childs) {
    if (childs != null) {
        childs.forEach(function (mnu) {
            var _mnu = dw_MenuToMenu(mnu);
...
var _MenuItems = [
    new Menu(1, 'Tableau de bord', '/dashboard', '', 'fa fa-tachometer', '', '', false, null, false, [])
];
payload.forEach(function (mnu) {
    var _mnu = dw_MenuToMenu(mnu);
    addChild(_mnu, orderBy(mnu.DW_MenuChilds, [{ field: 'IndexOrder', dir: 'asc' }]));
    _MenuItems.push(_mnu);
});
export var MenuItems = _MenuItems;
